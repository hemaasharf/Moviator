...
export const Header = () => {
  ...
  const [darkMode, setDarkMode] = useState(false || JSON.parse(localStorage.getItem("dark")))
  ...
  // for control search item in searchBar
  const[qureyTerm, setQueryTerm] = useState('')
  const navigate = useNavigate()
  const handlSubmit = (e)=>{
    e.preventDefault()
    setQueryTerm('')
     return navigate(`/search?q=${qureyTerm}`)
  }
  ...
  return (
    <header>
      <nav
        className="bg-white border-b-2 border-gray-200 px-2 sm:px-4 py-2.5 dark:bg-gray-900 dark:border-b-1 dark:border-gray-900">
        <div className=" flex flex-wrap justify-between items-center mx-auto">
          {/* this Link for Logo & name  */}
          ...
          <div id="mobile-nav" className="flex md:order-2">
            ...
            <div className="hidden relative md:block">
              ...
              <form onSubmit={handlSubmit}>
                <input onChange={handleChange} value={qureyTerm} type="text" id="search-navbar" className="block p-2 pl-10 w-full text-gray-900 bg-gray-50 rounded-lg border border-gray-300 sm:text-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search..." autoComplete="off" />
              </form>
            </div>
          ...
          {/* this section for navigation list  */}
